...
public partial class RequestTab : ObservableObject
{
    ...
    [ObservableProperty] private string _responseCookies = string.Empty;
    ...
    public async Task Send()
    {
        ...
        try
        {
            using var request = BuildHttpRequestMessage(requestUri);
            using var response = await SharedHttpClient.SendAsync(request);

            var responseBytes = await response.Content.ReadAsByteArrayAsync();
            watch.Stop();

            ResponseStatus = $"{(int)response.StatusCode} {response.ReasonPhrase}".Trim();
            ResponseTime = $"{watch.ElapsedMilliseconds} ms";
            ResponseSize = $"{responseBytes.Length} B";
            ResponseBody = DecodeResponseBody(response, responseBytes);
            ResponseHeaders = BuildResponseHeadersText(response);
            ResponseCookies = BuildResponseCookiesText(response);
            ResponseTabIndex = 0;
            HasResponse = true;
        }
        catch (Exception ex)
    ...
    private HttpRequestMessage BuildHttpRequestMessage(Uri requestUri)
    {
        var method = new HttpMethod(SelectedMethod.Name);
        var request = new HttpRequestMessage(method, requestUri);

        var editableHeaders = HeadersTable.ToNamedValues();
        AppendAuthorizationHeader(editableHeaders);

        var bodyText = ResolveRequestBodyText();
        var hasBody = method != HttpMethod.Get && method != HttpMethod.Head && !string.IsNullOrEmpty(bodyText);
        if (hasBody)
            request.Content = new StringContent(bodyText!, Encoding.UTF8);

        foreach (var h in editableHeaders.Where(h => h.Enabled && !string.IsNullOrWhiteSpace(h.Key)))
        {
            var key = h.Key.Trim();
            var value = h.Value;
    ...
    private string? ResolveRequestBodyText()
    {
        ...
        var relativePath = trimmed[2..].Trim();
        if (string.IsNullOrWhiteSpace(relativePath))
            return string.Empty;

        var baseDir = _collection is null
            ? Directory.GetCurrentDirectory()
            : Path.GetDirectoryName(_collection.FilePath) ?? Directory.GetCurrentDirectory();
        var fullPath = Path.GetFullPath(Path.Combine(baseDir, relativePath));
        return File.Exists(fullPath) ? File.ReadAllText(fullPath) : string.Empty;
    }

    private static string DecodeResponseBody(HttpResponseMessage response, byte[] bytes)
    {
    ...
    private static string BuildResponseHeadersText(HttpResponseMessage response)
    {
        var sb = new StringBuilder();
        foreach (var h in response.Headers)
            sb.Append(h.Key).Append(": ").AppendJoin(", ", h.Value).AppendLine();
    ...
    private static string BuildResponseCookiesText(HttpResponseMessage response)
    {
